...
\subsection*{Bảng so sánh tổng hợp giữa baseline resize và no-resize}
Để người đọc dễ theo dõi, nhóm tóm tắt các khác biệt chính giữa hai hướng thực nghiệm trong Bảng~\ref{tab:resize_noresize_compare}.


\begin{table}[H]
\centering
\caption{So sánh chi tiết giữa baseline resize và option no-resize.}
\label{tab:resize_noresize_compare}
\scriptsize
\begin{tabularx}{\textwidth}{|p{2.55cm}|Y|Y|}
\hline
\thead{Hạng mục} & \thead{Baseline resize 256} & \thead{No-resize original} \\
\hline
...
\hline
Input & Resize ảnh/mask về \(256 \times 256\), dễ batch và ổn định bộ nhớ. & Giữ ảnh gốc, không resize/padding/cap cạnh dài; batch size = 1. \\
\hline
...
\hline
Dữ liệu test & \important{613 ảnh}: 108 fracture, 505 normal. & \important{160 ảnh}: 23 fracture, 137 normal. \\
\hline
Postprocess & Threshold 0.08, lọc component nhỏ và dùng \textit{image-level gate}. & Threshold 0.03, không dùng gate để tránh xoá nhầm fracture nhỏ. \\
\hline
Kết quả chính & Dice full 0.369738; Dice positive \important{0.496755}; Recall positive 0.565802. & Dice all 0.856250 nhưng Dice positive \important{0.000000}; 23/23 fracture bị false negative. \\
\hline
...
\hline
Kết luận & Phù hợp làm \important{baseline chính} vì chạy đủ test và metric đáng tin hơn. & Chỉ nên xem là \important{option thử nghiệm}, cần thêm tài nguyên để đánh giá công bằng hơn. \\
\hline
\end{tabularx}
\end{table}

Subset no-resize gồm 137 ảnh \textit{normal} và 23 ảnh \textit{fracture}. Kết quả có 137 \textit{normal true negative} và 23 \textit{fracture false negative}. Nói cách khác, \important{tất cả ảnh fracture trong subset đều bị dự đoán rỗng}. Vì vậy Dice/IoU/Precision/Recall trên nhóm ảnh có mask thật đều bằng 0. Con số \code{Dice all = 0.856250} nhìn qua có vẻ cao, nhưng \important{không thể xem là kết quả tốt}; nó chủ yếu đến từ việc mô hình dự đoán rỗng đúng trên nhiều ảnh normal.
